...
import matplotlib
matplotlib.use('Agg')  # Arka planda sessiz grafik üretimi için
...
# GPU (Intel Arc 140V vb.) Sürücü Desteği
...
# -------------------------------------------------
# 4. PROMPTS.JSON OKUMA VE AKILLI ROTALAMA
# -------------------------------------------------
    ...
    try:
        if os.path.exists(PROMPTS_PATH):
            with open(PROMPTS_PATH, 'r', encoding='utf-8') as f:
                data = json.load(f)
                return data.get('master_system_prompt', '')
        else:
            print(f"[GK AI Uyarısı] prompts.json bulunamadı: {PROMPTS_PATH}")
            return "Sen, GK YEREL AI asistanısın."
    except Exception as e:
        print(f"[GK AI Hata] prompts.json okuma hatası: {e}")
        return "Sen, GK YEREL AI asistanısın."
...
# -------------------------------------------------
# 5. GRAFİK ÇALIŞTIRMA MOTORU
# -------------------------------------------------
def execute_chart_code(text):
    try:
        ...
        # plt.show() varsa plt.savefig() ile değiştir
        code = code.replace("plt.show()", f"plt.savefig(r'{chart_path}', bbox_inches='tight')")
        if "plt.savefig" not in code:
            code += f"\nplt.savefig(r'{chart_path}', bbox_inches='tight')"
        ...
        print(f"[GK AI Grafik Hatası]: {e}")
        return None

# -------------------------------------------------
# 6. MODEL AKIŞLARI VE GÖREV DEVRİ
# -------------------------------------------------
...
def stream_chat_response(prompt, model, images, history):
    def event_stream():
        ...
        # Yanıt bittiğinde arka planda grafik kuralını kontrol et ve çalıştır
        chart_url = execute_chart_code(accumulated_text)
        if chart_url:
            yield json.dumps({'type': 'chunk', 'text': f"\n\n![Grafik]({chart_url})"}, ensure_ascii=False) + '\n'
